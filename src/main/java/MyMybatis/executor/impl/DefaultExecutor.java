package MyMybatis.executor.impl;
...
public class DefaultExecutor implements Executor {
    ...
    public DefaultExecutor(Configuration config) {
        try {
            ...
            this.connection = DriverManager.getConnection(config.getJdbcUrl(), config.getJdbcUserName(), config.getJdbcPassword());
    ...
    public <E> List<E> query(MappedStatement mappedStatement, Object[] parameters) {
        ...
        try {
            ...
            handleResult(resultSet, result, mappedStatement.getResultType());
        } catch (SQLException e) {
            e.printStackTrace();
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
    ...
    @Override
    public int update(MappedStatement mappedStatement, Object[] parameters) {
        //7.Executor基于jdbc访问数据库获取数据
        int result = 0;
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(mappedStatement.getSql());
